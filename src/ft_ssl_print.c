...
/*   Updated: 2023/03/11 02:48:29 by cdarrell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void pre_print_hash(t_ssl *ssl, t_hash *hash)
{
	...
	else if (ssl->q == 0 && hash->type == 0 && ft_lstsize(ssl->hash_list) == 1 && ssl->s == 0)
		printf("(stdin)= ");
	else if (ssl->q == 0 && hash->type == 1 && ssl->r == 0)
		printf("%s (%s) = ", ssl->hash, hash->name);
	else if (ssl->q == 0 && hash->type == 0 && ssl->s == 1 && ssl->p == 0 && ssl->r == 0)
	{
		printf("%s (\"%s\") = ", ssl->hash, hash->name);
		ssl->s = 0;
	}
